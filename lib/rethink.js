...
.then(function (connection) {
    ...
    module.exports.findFanNumbers = function (truckid){
    ...
    module.exports.findSearchedTrucks = function (searchTerm){
      return rdb.table('trucks').filter(
        rdb.row("name").match("(?i).*"+searchTerm+".*")
      .or(
      //   rdb.row("yelpInfo")("categories")("").contains(".*"+searchTerm+".*")
      // ).or(
        rdb.row("promo").match("(?i).*"+searchTerm+".*")
      ).or(
        rdb.row("description").match("(?i).*"+searchTerm+".*")
      )).run(connection)
      .then(function (cursor) {
        return cursor.toArray();
      });
    }
// rdb.row("yelpInfo")("first").match(".*"+searchTerm+".*")
